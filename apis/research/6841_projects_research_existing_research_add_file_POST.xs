// Adds a file in the RESEARCH_Files_ table for the Existing Research section
query "projects/research/existing-research/add-file" verb=POST {
  api_group = "Research"
  auth = "USER"
  ...
  stack {
    ...
    db.query RESEARCH_Files_ {
      where = $db.RESEARCH_Files.file.name == $file_created.name && $db.RESEARCH_Files.intervention_id == $input.intervention_id && $db.RESEARCH_Files.subsection == "existing"
      return = {type: "exists"}
    } as $EXISTS
