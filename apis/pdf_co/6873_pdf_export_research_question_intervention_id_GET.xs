query "pdf/export/research-question/{intervention_id}" verb=GET {
  api_group = "PDF.co"
  auth = "USER"
  ...
  stack {
    ...
    precondition (($RESEARCH_QUESTION|is_empty) == false && ($INTERVENTION|is_empty) == false) {
      error_type = "notfound"
    ...
    var $QUESTIONS {
      value = $INTERVENTION
        ...
        |set:"questions":$RESEARCH_QUESTION
    }
  
    var $FILE_NAME {
      value = "ResearchQuestions_ProjectName_InterventionName_Date.pdf"
        |replace:"ProjectName":$QUESTIONS.project_name
        |replace:"InterventionName":$QUESTIONS.intervention_name
        |replace:"Date":$QUESTIONS.generated_date
    }
    ...
    var $FOOTER {
      value = "<div class='preserveHtml' style='width:100%; padding:0px 80px 10px 80px; display: flex;justify-content: space-between;align-items: center;color: #B6BFC9;font-weight: 400;font-style: Regular;font-size: 8px;line-height: 150%;letter-spacing: 0%;'><span>project_name / intervention_name</span><span>&lt;<span class='pageNumber'></span>&gt;</span></div>"
        |replace:"project_name":$QUESTIONS.project_name
        |replace:"intervention_name":$QUESTIONS.intervention_name
    }
  
    function.run "PDFco/Request PDF from Template" {
      input = {
        template_name: "Behavioral Questions Export"
        data         : $QUESTIONS
        name         : $FILE_NAME
        footer       : $FOOTER
      }
