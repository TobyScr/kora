...
export async function GET(
  ...
  try {
    const data = await xanoGet(
      `/api:HWnSNCdS/projects/${id}/assumption-testing/assumption`,
      token
    );
    return NextResponse.json(data);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to fetch assumptions";
...
export async function POST(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to create assumption";
